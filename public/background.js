...
const runTwipChatboxAutosave = async (tabId, tab, runOption = true) => {
    if (runOption) {
        if (isVaildTwipChatboxSettingsPage(tab.url)) {
            ...
            await chromeConfirm(tabId, "현재 탭이 Twip Chatbox 설정페이지가 아니기 때문에 자동저장을 실행할 수 없습니다.\nTwip Chatbox 설정페이지로 이동하시겠습니까?", () => {
                chrome.tabs.update({ url: "https://twip.kr/dashboard/chatbox" });
            });
        }
    }
    else {
        ...
        case "twip-autosave":
            ...
            if (tabStatus[tabId.toString()] === "closed") {
                ...
                if (customThemeSource) {
                    ...
                    tmp[autosaveLocalStorageKey] = customThemeSource;
                    ...
                    await chrome.storage.local
                        ...
                        if (!TWIP_AUTOSAVED_OVERLAYS)
                            await chrome.storage.local.set({
                                TWIP_AUTOSAVED_OVERLAYS: [autosaveLocalStorageKey],
                            });
                        else if (!TWIP_AUTOSAVED_OVERLAYS.includes(autosaveLocalStorageKey))
                            await chrome.storage.local.set({
                                TWIP_AUTOSAVED_OVERLAYS: [
                                    ...TWIP_AUTOSAVED_OVERLAYS,
