...
const createTwipChatBoxAutosaveInterval = async (tabId, tab, twipChatboxID) => {
...
const runTwipChatboxAutosave = async (tabId, tab, runOption = true) => {
    if (runOption) {
        if (tab.url.match(/^http(s?)\:\/\/twip\.kr\/dashboard\/chatbox.*$/)) {
            ...
            if (!twipChatboxAutosaveIntervals[twipChatboxID]) {
                await createTwipChatBoxAutosaveInterval(tabId, tab, twipChatboxID);
            }
            else {
    ...
    if (sender.id) {
        switch (request.type) {
            ...
            case "twip-chatbox-set-autosave":
                console.log(request.tab.url, isVaildTwipChatboxSettingsPage(request.tab.url));
                break;
        }
    }
});
